  ...
  static class FormatForDatastore extends DoFn<KV<String, List<CompletionCandidate>>, Entity> {
    ...
    public void processElement(ProcessContext c) {
      ...
      for (CompletionCandidate tag : c.element().getValue()) {
        ...
        tagEntity.getProperties().put("tag", makeValue(tag.value).build());
        tagEntity.getProperties().put("count", makeValue(tag.count).build());
        candidates.add(makeValue(tagEntity).setExcludeFromIndexes(true).build());
      }
      entityBuilder.getProperties().put("candidates", makeValue(candidates).build());
